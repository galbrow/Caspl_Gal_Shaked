...
myCalc:
    ...
    ;enter number  
    mov word[acccumulator],0            ;the accumulator value
    push 5
    call malloc
    add esp, 4
    ...
    shr word [num],word [counter]            ;ask peter about word/dword
    mov edx, [acccumulator]
    add dword [num], edx                ;ask peter about word/dword
    mov word [acccumulator], 0               ;reset accumulator
    mov edx,dword[num];need to check how to pass only first 8 bytes to ebx
    ...
    mov edx, [acccumulator]
    add dword [num], edx                ;ask peter about word/dword
    mov word[acccumulator], 0                          ;reset edx
    mov byte [ebx],byte [num]           ;node.value = 10101111 --> AF
    shl dword[num],8                         ;shift num 8 bytes to get the leftmost bytes
    mov edx, dword [num]                 ;edx(counter) = 0
    ...
    add esp, 4                          ;fix esp
    mov edx, dword[acccumulator]
    mov dword [eax],edx                 ;mov to the node the acccumulator edx value
