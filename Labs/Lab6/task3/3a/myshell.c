...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        if (arr[0] == '!')
        {
            if (atoi(&arr[1]) > j || arr[1] < '0' || arr[1] > '9')
            {
                fprintf(stderr, "invalid input! \n");
                continue;
            }
            else
                strcpy(arr, historyArray[atoi(&arr[1])]);
        }
        cmdLine *cl = parseCmdLines(arr); //parse---->
        j++;
        if (strcmp("history", cl->arguments[0]) == 0)
        {
            freeCmdLines(cl);
            int index = 0;
            while (index < j)
            {
                printf("%d) %s", index, historyArray[index]);
                index++;
            }
            historyArray[j] = malloc(strlen(arr));
            strcpy(historyArray[j], arr);
            continue;
        }
        historyArray[j] = malloc(strlen(arr));
        strcpy(historyArray[j], arr);
